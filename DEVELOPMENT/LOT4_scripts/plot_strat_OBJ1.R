...
pattern1 = c('incidence', "prevalence", "discontinued")

ARS <- list.files(path=paste0(my_path,"/ARS"), pattern=paste0(pattern1, collapse="|"), ignore.case = T)

BIFAP <- list.files(path=paste0(my_path,"/BIFAP"), pattern=paste0(pattern1, collapse="|"), ignore.case = T)

CASERTA <- list.files(path=paste0(my_path,"/CASERTA"), pattern=paste0(pattern1, collapse="|"), ignore.case = T)

CPRD <- list.files(path=paste0(my_path,"/CPRD"), pattern=paste0(pattern1, collapse="|"), ignore.case = T)

FISABIO <- list.files(path=paste0(my_path,"/FISABIO"), pattern=paste0(pattern1, collapse="|"), ignore.case = T)

PHARMO <- list.files(path=paste0(my_path,"/PHARMO"), pattern=paste0(pattern1, collapse="|"), ignore.case = T)

my_names<-c("ARS", "BIFAP", "CASERTA", "CPRD", "FISABIO", "PHARMO")

obj_1_list<-list(ARS, BIFAP, CASERTA, CPRD, FISABIO, PHARMO)


for(j in 1:length(my_names)){
  dap_path<-(paste0(my_path,"/", my_names[j]))  
  OBJ1_tables<-obj_1_list[[j]]
  
for(i in 1:length(OBJ1_tables)){
  my_data<-readRDS(paste0(dap_path,"/",OBJ1_tables[i]))
  
  main_name<-substr(OBJ1_tables[i], 1,nchar(OBJ1_tables[i])-4)
  
  main_name<-paste0(my_names[j],"_", main_name)
  
  pdf((paste0(my_path,"/Obj_1/plot_age/", main_name, ".pdf")), width=14, height=7)
...
obj_1_list<-list(ARS, BIFAP, CPRD, PHARMO)


for(j in 1:length(my_names)){
  dap_path<-(paste0(my_path,"/", my_names[j]))
  OBJ1_tables<-obj_1_list[[j]]
  OBJ1_tables<-str_subset(OBJ1_tables, "Valp")
for(i in 1:length(OBJ1_tables)){
  my_data<-readRDS(paste0(dap_path,"/", OBJ1_tables[i]))
  ...
  pdf((paste0(my_path,"/Obj_1/plot_indication/", main_name, ".pdf")), width=14, height=7)
