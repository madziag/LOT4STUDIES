...
# Total folders in All_regions_dir # 513 
my_folders <- list.files(All_regions_dir, pattern="counts") # 508 (excluded: 2 baseline tables, 2 denominators, 2 plots)
...
for(i in 1:length(my_folders_props)){
  ...
  my_data[!is.finite(my_data$rates),]$rates <- 0  #Set NA/inf rate values to 0
  my_max <- (max(my_data$rates))*1.1
  main_name<-paste0("Pooled_rate_",substr(my_folders_props[i], 1,nchar(my_folders_props[i])-7))
  pdf((paste0(bifap_plots, main_name,".pdf")), width=8, height=4)
  plot(x=1:nrow(my_data),y=my_data$rates, ylim=c(0,my_max), main=main_name, type = "n",xaxt="n", xlab="", ylab="rates")
  lines(x=(1:nrow(my_data)), y=my_data$rates, lwd=2)
  axis(1, at=1:nrow(my_data), as.character(my_data$YM), las=2)
...
##########################################################################################################################
############################################## PLOTS FOR STRATIFIED ANALYSIS ######################################################
##########################################################################################################################

# Combined plots 
for(i in 1:length(my_folders_stratified )){
  #read in list of RDS files
  my_files <-grep(list.files(path=paste0(All_regions_dir,my_folders_stratified [i])), pattern='Pooled', invert=TRUE, value=TRUE)
  pool_file<-list.files(path=paste0(All_regions_dir,my_folders_stratified [i]), pattern='counts_Pooled')
  
  my_pool<-fread(paste0(All_regions_dir, my_folders_stratified [i],"/", pool_file ))
  my_max<- (max(my_pool$rates))*2
  if (length(my_files)>0){
    main_name<-paste0("Each_region_prop_",substr(my_folders_stratified [i], 1,nchar(my_folders_stratified [i])-7))
    pdf((paste0(bifap_plots, main_name,".pdf")), width=8, height=4)
    plot(x=1:nrow(my_pool),y=rep(1, nrow(my_pool)), ylim=c(0,1), main=main_name, type = "n",xaxt="n", xlab="", ylab="proportions")
    axis(1, at=1:nrow(my_pool), as.character(my_pool$YM), las=2)
    # axis(1, at=1:nrow(my_date_df), as.character(my_pool$YM), las=2)
    ...
    legend("topleft", legend=c("true values", "no observations"), bty="n",pch=c(16,3))
    for(j in 1:length(my_files)){
      my_data<-fread(paste0(All_regions_dir,my_folders_stratified [i],"/",my_files[j]))
      my_data[!is.finite(my_data$rates),]$rates <- 0  #Set NA/inf rate values to 0
      my_reg<-substr(my_files[j],1,2)
      my_col<-as.character(region_key$my_pallette[region_key$my_regions==my_reg])
      #jitterred lines so that overlapping regions are distinguishable
...
for(i in 1:length(my_folders_stratified )){
  my_files<-grep(list.files(path=paste0(All_regions_dir,my_folders_stratified [i])), pattern='Pooled', invert=FALSE, value=TRUE)
  my_data<-my_data<-fread(paste0(All_regions_dir,my_folders_stratified [i],"/",my_files))
  my_data[!is.finite(my_data$rates),]$rates <- 0  #Set NA/inf rate values to 0
  my_max <- (max(my_data$rates))*1.1
  main_name<-paste0("Pooled_rate_",substr(my_folders_stratified [i], 1,nchar(my_folders_stratified [i])-7))
  pdf((paste0(bifap_plots, main_name,".pdf")), width=8, height=4)
  plot(x=1:nrow(my_data),y=my_data$rates, ylim=c(0,my_max), main=main_name, type = "n",xaxt="n", xlab="", ylab="rates")
  lines(x=(1:nrow(my_data)), y=my_data$rates, lwd=2)
  axis(1, at=1:nrow(my_data), as.character(my_data$YM), las=2)
