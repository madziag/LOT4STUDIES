...
readRDS(paste0(base_folder, "/g_intermediate/populations/ALL_study_population.rds"))

start.date<-as.Date(ALL_study_population$start_follow_up)
end.date<-as.Date(ALL_study_population$end_follow_up)
...
saveRDS(data.table(FUmonths_df), paste0(base_folder, "/g_intermediate/populations/denominator.rds"))
write.csv(FUmonths_df,paste0(base_folder, "/g_intermediate/populations/denominator.csv"))

pdf((paste0(base_folder, "/g_output/plots/denominator.pdf")), width=8, height=4)
