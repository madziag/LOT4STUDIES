...
incidence_all_counts<-readRDS(paste0(medicines_counts_dir,"/ALL_Retinoid_incidence_counts.rds"))
# Read in retinoid incidence file
df_incidence<-readRDS(paste0(g_intermediate,"counts_dfs/objective_1/ALL_Retinoid_incidence_counts_df.rds"))
# Performs incidence counts - stratified by ATC (for retinods)
...
for(group in 1:length(ATC_unique)){
  ...
  saveRDS(incidence_ATC_counts,(paste0(medicines_counts_dir,"/ALL_Retionoid_ATC_", ATC_unique[group],"_incidence_counts.rds")))
  write.csv(incidence_ATC_counts,(paste0(medicines_counts_dir,"/ALL_Retionoid_ATC_", ATC_unique[group],"_incidence_counts.csv")))
}

### PREVALENCE 
# Load all prevalent counts
prevalence_all_counts<-readRDS(paste0(medicines_counts_dir,"/ALL_Retinoid_prevalence_counts.rds"))
# Read in retinoid incidence file
df_prevalence<-readRDS(paste0(g_intermediate,"counts_dfs/objective_1/ALL_Retinoid_prevalence_counts_df.rds"))
...
for(group in 1:length(ATC_unique)){
  ...
  # Saves files in medicine counts folder
  saveRDS(prevalence_ATC_counts,(paste0(medicines_counts_dir,"/ALL_Retionoid_ATC_", ATC_unique[group],"_prevalence_counts.rds")))
  write.csv(prevalence_ATC_counts,(paste0(medicines_counts_dir,"/ALL_Retionoid_ATC_", ATC_unique[group],"_prevalence_counts.csv")))
}


# Create folder inside medicines folder for csv or excel file format
invisible(ifelse(!dir.exists(paste0(medicines_counts_dir,"/retinoids_by_ATC/")), dir.create(paste0(medicines_counts_dir,"/retinoids_by_ATC")), FALSE))
medicines_counts_add <- paste0(medicines_counts_dir,"/retinoids_by_ATC")

for(file in list.files(medicines_counts_dir,pattern="_ATC_",ignore.case = F)){file.copy(paste0(medicines_counts_dir, "/", file),medicines_counts_add)}
for(file in list.files(medicines_counts_dir,pattern="_ATC_",ignore.case = F)){unlink(paste0(medicines_counts_dir, "/", file))}
