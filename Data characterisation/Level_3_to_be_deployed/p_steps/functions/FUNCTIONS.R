...
INPUTMATRIX  <- function(d,value,type,var,var.v,cat = NULL,cat.v = NULL, per = F, perdir = "row"){
  ...
  if(type == "none"){graph <- d[,.(get(var),get(cat),get(value))]}
...
Line_plot2 <- function(MATRIX,title,x.l,y.l,y.axis = NULL,color = NULL){
  ...
  #X-axis
  if(is.null(y.axis)) {axis(1,at=1:length(colnames(MATRIX)),labels = colnames(MATRIX), las=2,cex.axis =0.6)}else{
  axis(1,at=y.axis,labels = as.character(y.axis), las=1,cex.axis =0.6)}
  ...
  legend("right",legend =  rownames(MATRIX), col = color, cex = 0.4,pch=10, box.col = "white")
