...
if(length(actual_tables$EVENTS)>0){
  ...
  for (y in 1:length(actual_tables$EVENTS)){
    ...
    df[,event_date:=as.Date(event_date,"%Y%m%d")] #transform to date variables
    ...
    events_out_st_per[[w]]<-df[event_date<start_follow_up | event_date>op_end_date,.N] #number of records outside study population
    df[(event_date<start_follow_up | event_date>op_end_date), obs_out:=1]
  ...
  for (i in 1:length(files)){
    ...
    saveRDS(combined_diagnosis_events, paste0(diag_pop,subpopulations_names[s], "/", names(files)[i],"_events_diagnoses.rds"))
      ...
      counts_events[[i]]<-combined_diagnosis_events[,.N,by=.(condition,event_vocabulary,meaning,truncated_code)]
  ...
  names(counts_events)<-c("event_definition","vocabulary","meaning","code_truncated","count")
  counts_events<-counts_events[,lapply(.SD,sum), by=.(event_definition,vocabulary,meaning,code_truncated),.SDcols="count"]
  total_events<-do.call(rbind,total_events)
  names(total_events)<-c("event_definition","total")
  total_events<-total_events[,lapply(.SD,sum), by=.(event_definition),.SDcols="total"]
...
if(length(actual_tables$MEDICAL_OBSERVATIONS)>0){
  ...
  for (y in 1:length(actual_tables$MEDICAL_OBSERVATIONS)){
    ...
    df<-df[!is.na(year)]
    #remove records that are outside the obs_period for all subjects
    mo_out_st_per[[w]]<-df[event_date<start_follow_up | event_date>op_end_date,.N] #number of records outside study population
    df[(event_date<start_follow_up | event_date>op_end_date), obs_out:=1]
    df<-df[is.na(obs_out)] #remove records outside study period
    df[,obs_out:=NULL] 
  ...
  if(empty_event_code.my[,.N]==0){
    ...
    tab20_mo<-merge(mo_study_population_my,empty_event_code.my, by=c("meaning","year"))
  ...
  for (i in 1:length(files)){
    ...
    saveRDS(combined_diagnosis_mo, paste0(diag_pop,subpopulations_names[s], "/", names(files)[i],"_mo_diagnoses.rds"))
      ...
      counts_mo[[i]]<-combined_diagnosis_mo[,.N,by=.(condition,event_vocabulary,meaning,truncated_code)]
  ...
  names(counts_mo)<-c("event_definition","vocabulary","meaning","code_truncated","count")
  counts_mo<-counts_mo[,lapply(.SD,sum), by=.(event_definition,vocabulary,meaning,code_truncated),.SDcols="count"]
  total_mo<-do.call(rbind,total_mo)
  names(total_mo)<-c("event_definition","total")
  total_mo<-total_mo[,lapply(.SD,sum), by=.(event_definition),.SDcols="total"]
...
if(length(actual_tables$SURVEY_OBSERVATIONS)>0){
  ...
  empty_event_code.my<-list()#number of records with empty so source value in the study population by meaning and year
  ##############################################################################
  ...
  for (y in 1:length(actual_tables$SURVEY_OBSERVATIONS)){
    ...
    df<-df[!is.na(year)]
    #remove records that are outside the obs_period for all subjects
    so_out_st_per[[w]]<-df[event_date<start_follow_up | event_date>op_end_date,.N] #number of records outside study population
    df[(event_date<start_follow_up | event_date>op_end_date), obs_out:=1]
    df<-df[is.na(obs_out)] #remove records outside study period
    df[,obs_out:=NULL] 
  ...
  if(empty_event_code.my[,.N]==0){
    ...
    tab20_so<-merge(so_study_population_my,empty_event_code.my, by=c("meaning","year"))
  ...
  if (length(conditions_files)>0){
    ...
    for (i in 1:length(files)){
      ...
      saveRDS(combined_diagnosis_so, paste0(diag_pop,subpopulations_names[s], "/", names(files)[i],"_so_diagnoses.rds"))
        ...
        counts_so[[i]]<-combined_diagnosis_so[,.N,by=.(condition,event_vocabulary,meaning,truncated_code)]
    ...
    names(counts_so)<-c("event_definition","vocabulary","meaning","code_truncated","count")
    counts_so<-counts_so[,lapply(.SD,sum), by=.(event_definition,vocabulary,meaning,code_truncated),.SDcols="count"]
    total_so<-do.call(rbind,total_so)
    names(total_so)<-c("event_definition","total")
    total_so<-total_so[,lapply(.SD,sum), by=.(event_definition),.SDcols="total"]
...
tab21<-tab21[,lapply(.SD,sum), .SDcols=c("count"), by=.(event_definition, vocabulary, meaning,code_truncated)]
total<-total[,lapply(.SD,sum), .SDcols=c("total"), by=.(event_definition)]
if (!is.null(total)){
#combine results if same event_definition+vocabulary+meaning+code_truncated combination exists
tab21<-merge(tab21,total,by="event_definition")
} else {tab21<-NULL}
