...
flowchart<-fread(paste0(diag_dir,subpopulations_names[a], "/Masked/", subpopulations_names[a],"_flowchart_masked.csv"))
datatable(flowchart, options = list(scrollX=T))
}
```
...
datatable(description, options = list(scrollX=T))
  ...
  description<-fread(paste0(diag_dir,subpopulations_names[a], "/Masked/", subpopulations_names[a],"_description_masked.csv"))
datatable(description, options = list(scrollX=T))
}
```
...
```{r echo=F}
if (subpopulations_present=="No"){
  if(sum(grepl(list.files(paste0(diag_dir,"Masked/")), pattern="tab20_masked.csv"))>0){
...
datatable(tab20, options = list(scrollX=T))
  ...
  print("This table cannot be produced based on the available data.")
    ...
    if(sum(grepl(list.files(paste0(diag_dir,subpopulations_names[a], "/Masked/")), pattern="tab20_masked.csv"))>0){
    tab20<-fread(paste0(diag_dir,subpopulations_names[a], "/Masked/", subpopulations_names[a],"_tab20_masked.csv"))
    if(tab20[,.N]>0){
datatable(tab20, options = list(scrollX=T))
    } else {
    print("This table cannot be produced based on the available data.") 
...
```{r echo=F}
if (subpopulations_present=="No"){
  if(sum(grepl(list.files(paste0(diag_dir,"Masked/")), pattern="tab21_masked.csv"))>0){
...
datatable(tab21, options = list(scrollX=T))
  ...
  print("This table cannot be produced based on the available data.") 
    ...
    if(sum(grepl(list.files(paste0(diag_dir,subpopulations_names[a], "/Masked/")), pattern="tab21_masked.csv"))>0){
    tab21<-fread(paste0(diag_dir,subpopulations_names[a], "/Masked/", subpopulations_names[a],"_tab21_masked.csv"))
    if(tab21[,.N]>0){
    setcolorder(tab21, c("event_definition","code_truncated","count","total","meaning","vocabulary","data_access_provider","data_source"))
datatable(tab21, options = list(scrollX=T))
    } else {
