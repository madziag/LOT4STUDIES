...
for (condition_ind in 1:length(diagnoses_files)){
  ...
  output<-CountPersonTime2(Dataset_events = unique(diag_file[,.(person_id, comb, event_date)]),
                           ...
                           Name_event = "comb",
                           Date_event = "event_date",
                           Rec_period = rep((Age_max-1)*365, length(outcomes_list)),
                           Age_bands = c(0,11,55),
                           print = F, 
                           check_overlap = F)
}
  ...
  output<-melt(output, id.vars=c("sex_at_instance_creation","meaning", "Ageband","year","Persontime"), measure.vars = colnames(output)[!colnames(output) %in% c("sex_at_instance_creation","meaning", "Ageband","year","Persontime")], variable.name = "combined_diagnoses")        
  setnames(output,"value","no_records")
  setnames(output, "Ageband", "age_band")
  setnames(output, "Persontime", "person_years")
  ...
  saveRDS(output, paste0(diag_tmp, names(diagnoses_files)[condition_ind], "_rates.rds"))
  rm(output)
}
rm(diagnoses_files)
...
#tab21/tab22
############################
diagnoses_files<-list.files(diag_tmp)
for (cond_ind in 1:length(diagnoses_files)){
  ...
  tab22<-tab22_sex
  rm(tab22_sex)
  tab22[,rate_per_1000:=round((no_records/person_years)*1000,4)]
...
tab22_files<-lapply(paste0(diag_tmp, tab22_files), readRDS)
tab22<-do.call(rbind,tab22_files)
