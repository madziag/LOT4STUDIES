...
datao <- read.csv("CPRD_ALL_Valproate_med_use_during_contraception_episodes_counts.csv")
datao<-datao[61:122,]
datao$time <- 1:62
...
model_final<-gls(yvar~time+level+trend,data = data_pi, correlation = corARMA(p=1,form =~time),method ="ML")
summary(model_final)
