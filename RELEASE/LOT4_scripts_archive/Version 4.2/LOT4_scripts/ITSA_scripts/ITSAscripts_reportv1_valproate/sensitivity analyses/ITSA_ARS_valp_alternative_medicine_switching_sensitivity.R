...
datao <- read.csv("ARS_ALL_Valproate_switched_to_alt_meds_counts.csv")
datao<-datao[61:122,]
datao$time <- 1:62
...
model_final<-gls(yvar~time+level+trend,data = data_pi, correlation = corARMA(p=5,form =~time),method ="ML")
summary(model_final)
