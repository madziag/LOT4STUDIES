...
if(length(altmeds_retinoids)>0){
  for(med in 1:length(altmeds_retinoids)){
    ...
    name_of_altmed<- gsub(".rds","", altmeds_retinoids[med])
...
if(length(altmeds_retinoids)>0){
  ...
  for(group in 1:length(alt_med_type_unique)){
    ...
    for(group_1 in 1:length(ATC_unique)){
      ...
      # Saves files in medicine counts folder
      saveRDS(altmed_indication_counts, paste0(medicines_counts_dir, "/", pop_prefix, "_alt_med_", alt_med_type_unique[group],"_ATC_", ATC_unique[group_1], "_counts.rds")) # Saves monthly counts 
    }
