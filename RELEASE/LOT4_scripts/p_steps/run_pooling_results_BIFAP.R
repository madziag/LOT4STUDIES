run_pooling <-  function(mask= T){
  ...
  # 2. Preg starts --- IS THIS A RATE OR A PROPORTION 
  ...
  for (i in 1:length(num_files)){
    if(str_detect(num_files[i], pattern = "discontinued")){num_subpop <- gsub("_discontinued_counts", "", num_files[i])}
    if(str_detect(num_files[i], pattern = "all_preg_starts")){num_subpop <- strsplit(num_files[i], "_all")[[1]][1]}
    if(str_detect(num_files[i], pattern = "switched")){num_subpop <- gsub("_switched_to_alt_meds_counts","", num_files[i])}
  ...
  for (i in 1:length(num_files)){
    ...
    if(str_detect(num_files[i], pattern = "switched")){num_file <- list.files(record_dir, pattern = "switched_to_alt_meds_counts_Pooled", full.names = T, ignore.case = T)}
