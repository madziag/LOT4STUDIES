...
if (nrow(D3_pregnancy_reconciled)>0){
  ...
  for (i in 1:length(med_files)){ 
    ...
    #### TESTING ####
    med_df[person_id=="ConCDM_SIM_200421_00247", person_id:="ConCDM_SIM_200421_00294"]
    med_df[person_id=="ConCDM_SIM_200421_00534", person_id:="ConCDM_SIM_200421_00596"]
    #### TESTING ####
    ...
    if (nrow(med_use_during_preg) > 0){
      ...
      # med_use_during_preg_counts <- med_use_during_preg[,.N, by = .(year(Date),month(Date))] # Performs counts grouped by year, month of medicine prescription date
      ...
      med_use_during_preg_counts[,masked:=0]
      # # Masking
      ...
      med_use_during_preg_counts <- med_use_during_preg_counts[,c("YM", "N", "Freq", "rates", "masked", "true_value")]
      # Save files 
      saveRDS(med_use_during_preg, paste0(pregnancy_related_dir, gsub("_MEDS.rds", "", med_files[i]), "_med_use_during_pregnancy.rds"))
      saveRDS(med_use_during_preg_counts, paste0(preg_med_counts_dir,"/", gsub("_MEDS.rds", "", med_files[i]), "_med_use_during_pregnancy_counts.rds"))
      ...
      for (j in 1:length(hq_unique)){
        ...
        # Masking is not applied before stratification
        med_use_during_preg_unique_counts[,masked:=0]
        # Masking
        ...
        med_use_during_preg_unique_counts[,masked:=ifelse(N<5 & N>0, 1, 0)]
        # Applies masking to numerator
        if(mask==T){med_use_during_preg_unique_counts[masked==1,N:=5]} else {med_use_during_preg_unique_counts[masked==1,N:=N]}
        med_use_during_preg_unique_counts[Freq>0&Freq<5,Freq:=5]
        # Calculates rates
        ...
        med_use_during_preg_unique_counts <- med_use_during_preg_unique_counts[,c("YM", "N", "Freq", "rates", "masked", "true_value")]
        # Save files 
        saveRDS(med_use_during_preg_unique, paste0(pregnancy_related_dir, gsub("_MEDS.rds", "", med_files[i]), "_hq_", hq_unique[j], "_med_use_during_pregnancy.rds"))
        saveRDS(med_use_during_preg_unique_counts, paste0(preg_med_counts_dir,"/",gsub("_MEDS.rds", "", med_files[i]), "_hq_", hq_unique[j], "_med_use_during_pregnancy_counts.rds"))
      }
    } else {
