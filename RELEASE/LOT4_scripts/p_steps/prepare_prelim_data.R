...
if(is_BIFAP){for(file in list.files(path=mo_pop,pattern=paste0(pop_prefix, "_pregtest"))){file.copy(paste0(mo_pop,file),pregtest_dir)}}
...
if(length(list.files(pregtest_dir, pattern=paste0(pop_prefix, "_preg")))>0){
  ...
  for(file in list.files(path=pregtest_dir,pattern="EVENTS|PROC|MEDICAL_OBSERVATIONS")){unlink(paste0(pregtest_dir,file))}
  #Saves as one file
