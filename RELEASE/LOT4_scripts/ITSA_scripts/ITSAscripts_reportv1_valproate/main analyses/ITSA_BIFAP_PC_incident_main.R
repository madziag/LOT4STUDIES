...
datao <- read.csv("BIFAP_PC_Valproate_incidence_counts_Pooled.csv")
...
model_final<-gls(yvar~time+level+trend,data = data_pi, correlation = corARMA(p=4,form =~time),method ="ML")
summary(model_final)

acf(residuals(model_final))
acf(residuals(model_final),type="partial")

#Checking the model               
model_p11<-update(model_final,correlation=corARMA(p=6,form = ~time))
