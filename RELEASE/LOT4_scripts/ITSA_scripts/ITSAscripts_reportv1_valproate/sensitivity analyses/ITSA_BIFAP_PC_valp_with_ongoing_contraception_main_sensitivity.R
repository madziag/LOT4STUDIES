...
#Date = 23.03.22 
DAP = "BIFAP_PC"
analysis = "valproate_users_using_contraceptives_sensitivity" 
...
setwd("Y:/research-valproate-pooled/Report v1.0/Objective 2/Counts tables/main analyses")
  ...
  datao <- read.csv("BIFAP_PC_Valproate_med_use_during_contraception_episodes_counts_final_Pooled.csv")
datao<-datao[61:122,]
datao$time <- 1:62
...
ymin <- 0
ymax <- 20
y_lab <- "Percent prescriptions with ongoing contraception" 
...
#model_final<-gls(yvar~time+level+trend,data = data_pi, correlation = corARMA(p=3,form =~time),method ="ML")
model_final<-gls(yvar~time+level+trend,data = data_pi,method ="ML")
summary(model_final)

#Checking the model               
model_p11<-update(model_final,correlation=corARMA(p=12,form = ~time))
anova(model_p11,model_final)

#Plot results#
setwd("Y:/research-valproate-pooled/Report v1.0/Objective 2/ITSA_plots/sensitivity analyses")
