  ...
  for (s in 1: length(subpopulations_names)){
   ...
   if (length(conditions_files_chronic)>0){
    ...
    for (j in 1:length(conditions_files_chronic)){
     if (subpopulations_present=="Yes"){
      if (length(conditions_files_chronic[[j]])>1){
        diagnoses<-readRDS(paste0(diag_pop, subpopulations_names[s],"/",conditions_files_chronic[[j]][1]))
        #create min date for each person and keep only that record
        diagnoses[, min_date:=min(event_date), by="person_id"]
        #keep only the earliest record for each person
        diagnoses<-diagnoses[event_date==min_date]
        diagnoses[,min_date:=NULL]
        l<-2
        while (l<=length(conditions_files_chronic[[j]])){
          diagnoses<-rbind(diagnoses,readRDS(paste0(diag_pop, subpopulations_names[s],"/",conditions_files_chronic[[j]][l])))
          diagnoses[, min_date:=min(event_date), by="person_id"]
          #keep only the earliest record for each person
          diagnoses<-diagnoses[event_date==min_date]
          diagnoses[,min_date:=NULL]
        ...
        diagnoses<-readRDS(paste0(diag_pop, subpopulations_names[s],"/",conditions_files_chronic[[j]])) #combine all files for one pregnancy stage
      }
    } else {
      if (length(conditions_files_chronic[[j]])>1){
        diagnoses<-readRDS(paste0(diag_pop,conditions_files_chronic[[j]][1]))
        #craete min date for each person and keep only that record
        diagnoses[, min_date:=min(event_date), by="person_id"]
        #keep only the earliest record for each person
        diagnoses<-diagnoses[event_date==min_date]
        diagnoses[,min_date:=NULL]
        l<-2
        while (l<=length(conditions_files_chronic[[j]])){
          diagnoses<-rbind(diagnoses,readRDS(paste0(diag_pop,conditions_files_chronic[[j]][l])))
          diagnoses[, min_date:=min(event_date), by="person_id"]
          #keep only the earliest record for each person
          diagnoses<-diagnoses[event_date==min_date]
          diagnoses[,min_date:=NULL]
    ...
    diagnoses<-diagnoses[sex_at_instance_creation=="F" & age_start_follow_up>=min_age_preg & age_start_follow_up<=max_age_preg,c("person_id","condition", "truncated_code","event_vocabulary","birth_date","op_end_date","start_follow_up","age_start_follow_up","year", "event_date")]
    #get min year present 
    if(diagnoses[,.N]>0){
    min_year<-as.IDate(paste0(min(diagnoses[!duplicated(year),year]),12,31), "%Y%m%d")#first year of having an event in the database
    conditions_present<-diagnoses[!duplicated(condition),condition]
    for (l in min(diagnoses[!duplicated(year),year]):year(date_creation)){
      for (k in 1: length(conditions_present)){
      #create filter variable if event_date before end of year then filter:=1
      saveRDS(diagnoses[year<=l & year(start_follow_up)<=l & year(op_end_date)>=l & condition==conditions_present[k]], paste0(poi_tmp,l,"_",conditions_present[k], "_prevalence.rds"))
      }
    }
    } else {conditions_present<-NULL}
    rm(diagnoses, conditions_present)
