...
flowchart<-fread(paste0(preg_dir,"Masked/", "flowchart_pregnancy_masked.csv"))
datatable(flowchart, options = list(scrollX=T))
} else {
flowchart<-fread(paste0(preg_dir,subpopulations_names[a], "/Masked/", subpopulations_names[a],"_flowchart_pregnancy_masked.csv"))
datatable(flowchart, options = list(scrollX=T))
}
```
...
```{r echo=F}
if (subpopulations_present=="No"){
description<-fread(paste0(preg_dir,"Masked/", "description_pregnancy_masked.csv"))
datatable(description, options = list(scrollX=T))
} else {
  description<-fread(paste0(preg_dir,subpopulations_names[a], "/Masked/", subpopulations_names[a],"_description_pregnancy_masked.csv"))
datatable(description, options = list(scrollX=T))
}
```
...
```{r echo=F}
if (subpopulations_present=="No"){
  if(sum(grepl(list.files(paste0(preg_dir,"Masked/")), pattern="tab17_pregnancy_masked.csv"))>0){
tab17<-fread(paste0(preg_dir,"Masked/", "tab17_pregnancy_masked.csv"))
if(tab17[,.N]>0){
datatable(tab17, options = list(scrollX=T))
    ...
    print("This table cannot be produced based on the available data.")
      ...
      if(sum(grepl(list.files(paste0(preg_dir,subpopulations_names[a], "/Masked/")), pattern="tab17_pregnancy_masked.csv"))>0){
    tab17<-fread(paste0(preg_dir,subpopulations_names[a], "/Masked/", subpopulations_names[a],"_tab17_pregnancy_masked.csv"))
    if(tab17[,.N]>0){
datatable(tab17, options = list(scrollX=T))
  } else {
      print("This table cannot be produced based on the available data.")
...
## 5. Pregnancy counts and percentage in women with at least one pregnancy code and 365 days of follow up

<br> 

```{r echo=F}
if (subpopulations_present=="No"){
  if(sum(grepl(list.files(paste0(preg_dir,"Masked/")), pattern="tab19_pregnancy_masked.csv"))>0){
tab19<-fread(paste0(preg_dir,"Masked/", "tab19_pregnancy_masked.csv"))
if (tab19[,.N]>0){
datatable(tab19, options = list(scrollX=T))
    ...
    print("This table cannot be produced based on the available data.")
        ...
        if(sum(grepl(list.files(paste0(preg_dir,subpopulations_names[a], "/Masked/")), pattern="tab19_pregnancy_masked.csv"))>0){
    tab19<-fread(paste0(preg_dir,subpopulations_names[a], "/Masked/", subpopulations_names[a],"_tab19_pregnancy_masked.csv"))
    if (tab19[,.N]>0){
datatable(tab19, options = list(scrollX=T))
  } else {
      print("This table cannot be produced based on the available data.")
